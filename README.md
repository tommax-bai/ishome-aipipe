...
**两样（面积 + 户型图）齐了 → 只说一句"我这就为你做设计，请稍等。"**
（`service.DESIGN_START_MESSAGES`）：不含任何假设，不复述他刚给的，不播报进度，不再向他要任何信息。
**这一轮由系统文案接管，模型写的回复整批作废**（`service._design_start_due` 两支：缺口由图补上
那一支连编排模型都不调，由面积补上那一支抽完事实作废它的回复）——"缺口为空还提问"从此是**结构**、
不是提示词纪律：模型没有产回复的位置，业主那头就不可能出现问号。判据是 9-01 写死的"真机再出现
一次就做"，9-07 第二次出现（"您方便说说家里常住几口人？"）。**射程只到这一轮**，此后照旧走模型。
