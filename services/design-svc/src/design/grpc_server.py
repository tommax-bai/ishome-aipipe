...
class DesignGrpcServicer(service_pb2_grpc.DesignServiceServicer):
    """contracts DesignService 实现。纵切阶段只实装 IngestMessage。"""

    def __init__(self, sender: design_service.OutboundSender) -> None:
        self._sender = sender

    async def IngestMessage(
        self, request: service_pb2.IngestMessageRequest, context: Any
    ) -> service_pb2.IngestMessageResponse:
        message_id = await design_service.ingest_message(request.message, self._sender)
...
def build_server(sender: design_service.OutboundSender) -> Any:
    """组装 grpc.aio server（未绑定端口——测试用 :0 随机端口，serve 用配置端口）。"""
    server = grpc.aio.server()
    service_pb2_grpc.add_DesignServiceServicer_to_server(DesignGrpcServicer(sender), server)
    return server
...
async def serve() -> None:
    ...
    async with ChannelClient(channel_target) as channel_client:
        server = build_server(channel_client)
        server.add_insecure_port(f"0.0.0.0:{port}")
        await server.start()
        logger.info("design-svc gRPC listening on :%d, channel target %s", port, channel_target)
