...
内容：
- **给最直接简要的回答，能一句话说完就只说一句**：只说他必须知道的。不必要的解释、
  客套铺垫、进度播报（"接下来我会…"）、把他刚说的话复述一遍，一概不说。
  上面那个 1 到 3 条是**上限不是指标**，不用凑
...
- **绝不播报进度、绝不给时间估计、绝不描述系统内部这会儿在做什么**：系统里有没有活在跑、
  跑到哪一步、还要多久，**只有业务侧的回执说得算，你一个字都看不到**。禁止"正在解析"
  "正在生成""处理中""马上就好""很快""预计 X 分钟"这类说法，也禁止"完成后会…"
  "解析后将…"这类替系统许下的后续承诺。他直接问"怎么样了""还要多久"也一样——
  **不知道就不编**，回应他在等这件事本身即可（同"绝不编造具体数字"那条：编出来的话不算话）
- 口头提到系统推断出来的尺寸时带"约"
- 用户问设计原因时给出真实依据
- 不承诺系统尚未具备的能力；你这一轮只负责回话与记下他说的信息，别的环节不归你交代
- 禁止任何免责、责任划分类表述
...
# **"不许编进度"这一条今天是提示词纪律，不是结构**——写在这儿是为了别把它当成已经拦住了。
#
# 成因（2026-09-08 真机 18:47—18:48）：业主发图后连问两次"怎么样了""大概要多久？"，模型写出
# "户型图已收到，正在解析中""正在解析结构与空间关系""预计2分钟内完成""解析后将自动生成…"，
# 而业务侧那三轮回执全是 `tasks=[]`——**一个活都没在跑**，四句话没有一句有事实支撑。
#
# **能做成结构的那半今天做了**：把授权它这么说的输入拿掉（`step` 里缺口为空那段不再写"推完了
# 会主动告诉他"、上面那条不再写"之后生成初步设计方案"）——它看不见就说不出，同"不给它看内部
# 字段名"、同"推导步看不见落点的值"。**剩下这半做不成结构的理由**：会话侧压根没有进度这个事实
# 可给（有没有活在跑只有业务侧回执说得算，而那份回执一轮只有一个布尔量级的信息），所以拦不住的
# 那部分只能是"模型自由文本里冒出来的说法"，唯一的结构形态是出站口按词表判定——**词表对自由
# 文本只拦得住它列举过的说法**，"马上就好""很快"漏一条，后果与今天一样；而它命中时的动作是把
# 业主的正经回答整条吞掉，代价落在业主身上。今天手上只有一份真跑样本，够不着"阈值有数据才定"。
#
# **判据写死（照 9-01 那条的形态）＝真机再出现一次进度承诺就做**，形态也写死：出站前对模型写的
# **逐条**判（进度播报 / 时间估计 / 替系统许诺后续三类），命中的那一条不发、其余照发，全被拿掉
# 就只发系统回执（同 service 里"两样齐那一轮模型的回复整批作废"那一支的形态）。到那时手上会有
# 第二份真跑样本，词表按样本立，不按想象立。**判据写死了就要照做**——9-07 的教训是等"再出现
# 一次"的代价由业主承担（见《落地假设与拍板清单》§〇 同日条目）。
...
async def step(
    ...
    *,
    ...
    context = ("已收集的信息：\n" + ("\n".join(state_lines) if state_lines else "（暂无）")) + (
        ...
        #
        # **这段话里不许出现"系统随后会做什么"**（2026-09-08）：首版结尾写的是"推完了会主动
        # 告诉他按什么做的"，模型读到之后就替系统许诺起了后续——真机 18:48 它写出"解析后将
        # 自动生成符合138㎡和81%得房率的初步设计方案"。**它看不见就说不出**：与其只加一条
        # "不许承诺进度"的禁令（那条也加了，在 `_SYSTEM_PROMPT` 里），不如先把授权它这么说的
        # 那句话拿掉——同"推导步看不见落点的值所以产不出数字"、同"不给它看内部字段名"。
        else (
            "\n\n**该要的都要到了：这一轮不要再向他要任何信息**，"
            "不要问人数、诉求、预算、小区名，也不要提出下一步要他配合的事。"
            "这些系统自己会推，不用他给。他想补充什么，等他自己说。"
            "**也不要交代系统这会儿在做什么、什么时候好**——那不是你知道的事。"
        )
    )
...
def confirm_ack_text() -> str:
    ...
    **不交代系统下一步做什么**（2026-09-08）：原文写着"这部分能力正在接入，完成后会直接发到
    这个对话里"——这是系统内部进度，会话侧不知道也不该说（同 `_SYSTEM_PROMPT` 里管住模型的
    那一条，一条规矩两边都得守）。这条路径今天不通（确认清单的时点已挪到真有产出可确认时），
    但它是写死的文案，醒过来的那天就会原样发给业主。
    """
    return (
